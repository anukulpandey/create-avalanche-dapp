...
// const [_AVALANCHE_MAINNET_PARAMS, set_AVALANCHE_MAINNET_PARAMS] = useState(AVALANCHE_TESTNET_PARAMS)
...
let AVALANCHE_NETWORK_PARAMS = AVALANCHE_TESTNET_PARAMS
...
export class OnboardingButton extends React.Component {
  ...
  render () {
    ...
    if (!MetaMaskOnboarding.isMetaMaskInstalled()) {
      ...
      return (
        <div>
          <button onClick={this.connectMetaMask}  className='des'>
            Connect your Wallet
          </button>
        </div>
      )
      ...
      return (
        ...
        <div className='error' >
          <h3 className='error-msg'>Error Encountered!</h3>
          <div>Account: {this.state.accounts[0]}</div>
          <div className='error-msg'>To run this dApp you need to switch to the {AVALANCHE_NETWORK_PARAMS.chainName} chain</div>
          <button className='btn' onClick={this.switchToAvalancheChain}>
