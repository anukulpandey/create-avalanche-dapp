...
const createDapp = (resolvedProjectPath, projectPath) => {
	...
	const timer = setInterval(function () {
		...
		if (value >= progressBar.getTotal()) {
			clearInterval(timer);
			fse.mkdtemp(path.join(os.tmpdir(), "avalanche-"), (err, folder) => {
				if (err) throw err;
				execSync(
					`git clone --depth 1 ${"https://github.com/anukulpandey/create-avalanche-dapp"} ${folder}`,
					{ stdio: "pipe" }
				);
...
const cmdLineRes = yargs(process.argv.slice(2)).command(
	"$0 [folder-name]",
	"create a new Avalanche dApp project",
	yargs => {
		yargs.positional("folder-name", {
			describe: "Folder to create your Avalanche dapp in",
			type: "string",
			default: null,
		});
	}
...
`)
	...
	console.log("\n");
	console.log("🪸 Welcome to the create-avalanche-dapp wizard 🪸");
